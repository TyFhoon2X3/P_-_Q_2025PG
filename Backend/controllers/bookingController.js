...
`,
  ...
  async create(req, res) {
    try {
      const { vehicle_id, date, time, description, transport_required } =
        req.body;
  ...
  async uploadSlip(req, res) {
    try {
      ...
      await pool.query(
       `UPDATE bookings SET slipfilename=$1 WHERE booking_id=$2`,
        [filename, id]
      );
  ...
  async updateCost(req, res) {
    ...
    try {
      ...
      if (!check.rowCount)
        return res
          .status(404)
          .json({ success: false, message: "ไม่พบบุ๊คกิ้งนี้" });
  ...
  async updateStatus(req, res) {
    ...
    try {
      ...
      await pool.query(`UPDATE bookings SET status_id=$1 WHERE booking_id=$2`, [
        status_id,
        id,
      ]);
  ...
  // ✅ อัปโหลดสลิป (ลูกค้าอัปโหลดหลังสถานะ = รอชำระเงิน)
  async uploadSlip(req, res) {
    try {
      const { id } = req.params;
      const filename = req.file?.filename;
    ...
    `,
      ...
      if (booking.status_id !== 5)
        return res.status(400).json({
          success: false,
          message: "อัปโหลดสลิปได้เฉพาะเมื่อสถานะเป็น 'รอชำระเงิน'",
        });

      // อัปเดตชื่อไฟล์ + เปลี่ยนสถานะเป็นเสร็จสิ้น (6)
      await pool.query(
        `UPDATE bookings 
       SET slipfilename=$1, status_id=5
       WHERE booking_id=$2`,
        [filename, id]
      );
